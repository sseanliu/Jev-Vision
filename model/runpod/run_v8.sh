#!/usr/bin/env bash
# V8: stage-3 from V7. Fixes aimed at the two open defects: the decider's element choice (every recorded target
...
RUN_NAME=${RUN_NAME:-v8-8b-decider}; LR=${LR:-1.5e-5}; HEAD_LR=${HEAD_LR:-1.5e-4}; SMOOTH=${SMOOTH:-0.05}
GEN_SAMPLE=${GEN_SAMPLE:-5000}; GQA_SAMPLE=${GQA_SAMPLE:-12000}; REPLAY_PIXEL=${REPLAY_PIXEL:-3000}; REPLAY_STATE=${REPLAY_STATE:-2000}; REPLAY_WEB=${REPLAY_WEB:-1500}; REPLAY_DESK=${REPLAY_DESK:-1000}
REPO=${REPO:-/workspace/jev-probes}; INIT=${INIT:-$REPO/model/runs/v7-8b-general/final}
V=/workspace/vision; D=/workspace/v8_data; mkdir -p $D; rm -f $D/*.jsonl
...
rng = random.Random(8); D = "/workspace/v8_data"; V = "/workspace/vision"
...
import json, glob, os
missing = total = 0
for f in glob.glob("/workspace/v8_data/*.train.jsonl"):
    for l in open(f):
        r = json.loads(l)
