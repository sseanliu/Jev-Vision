...
REPLAY_WEB=${REPLAY_WEB:-4000}; REPLAY_DESK=${REPLAY_DESK:-1500}; LR=${LR:-4e-5}; HEAD_LR=${HEAD_LR:-4e-4}
...
REPLAY_WEB=$REPLAY_WEB REPLAY_DESK=$REPLAY_DESK python - <<'PY'
import json, random, os
rng = random.Random(5); nw = int(os.environ["REPLAY_WEB"]); nd = int(os.environ["REPLAY_DESK"])
web = [l for l in open("/workspace/m2w_schema/schema.train.jsonl")]; open("/workspace/v5_data/replay_web.train.jsonl", "w").writelines(rng.sample(web, min(nw, len(web))))
desk = [l for l in open("/workspace/v3_data/osatlas_schema.train.jsonl")]; open("/workspace/v5_data/replay_desktop.train.jsonl", "w").writelines(rng.sample(desk, min(nd, len(desk))))
PY
wc -l $D/*.jsonl
cd model
if [ ! -f $INIT/eval_state_zeroshot.json ]; then
