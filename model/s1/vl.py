...
class VLPacker:
    ...
    def pack(self, image, state_text: str, questions: list[Question],
             max_pixels: int | None = None) -> PackedVL:
        """image: one PIL image / path, or a list of them (e.g. before and after screenshots). Every image
        goes into the state prefix in order, each as <|vision_start|>...<|vision_end|>."""
        imgs = image if isinstance(image, (list, tuple)) else [image]
        imgs = [Image.open(i).convert("RGB") if isinstance(i, str) else i for i in imgs]
        ...
        enc = self.proc(text=[IMAGE_PREFIX * len(imgs) + state_text], images=imgs, return_tensors="pt",
...
class VLDecisionModel(DecisionModel):
    ...
    def _mrope_positions(self, ids: torch.Tensor, pos1d: torch.Tensor, seg: torch.Tensor,
                         state_lens: list[int], mm_tt: list[list[int]], grid: torch.Tensor,
                         n_images: list[int] | None = None) -> torch.Tensor:
        ...
        for b in range(B):
            ...
            n_img = n_images[b] if n_images is not None else int((tt == 1).any())
    ...
    def hidden_vl(self, ids, seg, pos1d, pixel_values, grid, state_lens, mm_tt, n_images=None):
        mask = tree_mask(seg, self.backbone.dtype)
        pos3 = self._mrope_positions(ids, pos1d, seg, state_lens, mm_tt, grid, n_images)
    ...
    def forward(self, packed: PackedVL, device=None):
        ...
        h = self.hidden_vl(ids, seg, pos, packed.pixel_values.to(dev, self.backbone.dtype),
                           packed.image_grid_thw.to(dev), [packed.state_len], [packed.mm_token_type_ids], [packed.n_images])[0]
...
def collate_vl(examples, pad_id: int):
    ...
    return ids, seg, pos, pix, grid, [e.packed.state_len for e in examples], [e.packed.mm_token_type_ids for e in examples], examples, [e.packed.n_images for e in examples]
